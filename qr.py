...
def index():
    ...
    if request.method == 'POST':
        ...
        if len(fio.split(" ")) == 3 and (int(day) > 0 and int(day) < 32):
            ...
            date = day + " " + month
            hash = hashlib.sha256(str(time.time()).encode('utf-8')).hexdigest()[:12].upper()
            with open(os.getcwd().replace("\\", "/") + "/templates/" + "qr-" + hash + ".html", "w") as f:
            ...
            url.png(os.getcwd().replace("\\", "/") + "/templates/static/" + 'qr-' + hash + '.png', scale=5)
        else:
            message = "Данные введены неверно"
            return render_template('index.html', message=message)
        return send_file(os.getcwd().replace("\\", "/") + "/templates/static/" + 'qr-' + hash + '.png', as_attachment=True, mimetype='image/png')
    else:
        return render_template('index.html', message=message)
